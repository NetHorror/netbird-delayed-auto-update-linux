...
- **default delay increased to 10 days**.
...
- **Default delay** for new NetBird versions is now **10 days**:
  - when you run `--install` without extra flags, the timer will only allow
    upgrades after the candidate version has stayed in the APT repo unchanged
    for at least 10 days;
  - you can override this with `--delay-days N` during installation.
- `state.json` handling no longer uses `python3`:
  - the script writes a small JSON file itself and reads it back via `grep`,
    which simplifies runtime dependencies;
  - Python is no longer required to run the delayed-update logic.
- Age calculation between `FirstSeenUtc` and the current time is clamped to
  **>= 0 days** to avoid negative values when the system clock moves backwards.
- Version comparison is now based on `dpkg --compare-versions`, which correctly
  handles Debian/Ubuntu-style version strings.
...
To keep the previous behaviour with a **shorter delay** (e.g. 3 days), install with:
